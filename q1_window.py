...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for i in range(len(sentence)):
            prev_win = [sentence[j] if j>=0 else start for j in range(i-window_size,i)]
            next_win = [sentence[j] if j<len(sentence) else end for j in range(i+1, i+window_size+1)]
            window = [feature for features in prev_win for feature in features] + \
                        sentence[i] + \
                        [feature for features in next_win for feature in features]
            windowed_data.append((window,labels[i]))
        # flat_list = start*window_size
        #
        # for word in sentence:
        #     flat_list = word + flat_list
        # flat_list = flat_list + end*window_size
        # D = 2*(2*window_size+1)
        # i=0
        # for x in range(0,D,2*window_size):
        #     print(x)
        #     windowed_data.append((flat_list[x:x+D],labels[i]))
        #     i=i+1
        #     print(windowed_data)
		### END YOUR CODE
