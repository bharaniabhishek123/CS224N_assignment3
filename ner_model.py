...
class NERModel(Model):
    ...
    def fit(self, sess, saver, train_examples_raw, dev_set_raw):
        ...
        for epoch in range(self.config.n_epochs):
            ...
            # mb,lab = minibatches(train_examples,batch_size=self.config.batch_size,shuffle=True)
            loss_=0
            for m,l in minibatches(train_examples,batch_size=self.config.batch_size,shuffle=True):
                loss_m = self.train_on_batch(self.sess,m,l)
                loss_=loss_+ loss_m
            ### END YOUR CODE
